...
1. **Onboarding** (`/ark:new`) — Claude Code slash command that walks users through designing experiments
...
1. **No API keys needed** — Claude Code IS the AI researcher. No separate LLM calls.
...
```
/ark:new (Claude Code slash command)
  ├── reads: templates/*
  ├── conversation: Claude Code designs the experiment
  └── writes: experiment_dir/
        ...
        └── [domain-specific files generated by Claude]
